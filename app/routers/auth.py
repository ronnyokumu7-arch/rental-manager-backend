...
def _get_valid_reset_token_or_400(token: str, db: Session) -> PasswordResetToken:
    ...
    # Handle timezone normalization for comparison
...
def login(credentials: LoginRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Accept legacy credentials that may still be stored as plaintext.
...
def reset_password(
    ...
    # Validate token and expiration via helper
    db_token = _get_valid_reset_token_or_400(payload.token, db)
    
    # Validate user via helper
